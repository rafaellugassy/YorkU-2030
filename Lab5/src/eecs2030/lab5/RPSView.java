...
public class RPSView extends JFrame {
	...
	JLabel player = new JLabel(); // a label describing the hand played by the
									// player
	JLabel computer = new JLabel(); // a label describing the hand played by the
									// computer
	JLabel winner = new JLabel(); // a label describing who won the last round
									// played
	JLabel record = new JLabel(); // a label describing the won-lost-draw record
									// of the rounds
	// played
	...
	private void makeLabels() {
		this.makeLabel(this.player, "You picked: ");
		this.makeLabel(this.computer, "Computer picked: ");
		this.makeLabel(this.winner, "No winner yet");
		this.makeLabel(this.record, "Games won: 0, Games lost: 0, Games drawn: 0");
	}
	...
	private void makeButtons(ActionListener listener) {
		makeButton(listener, this.rock, RPSUtils.ROCK);
		makeButton(listener, this.paper, RPSUtils.PAPER);
		makeButton(listener, this.scissors, RPSUtils.SCISSORS);
	}

	private void makeButton(ActionListener listener, JButton button, String text) {
		button = new JButton();
		button.setText(text);
	...
	public void setWinner(String winner) {
		if (winner == RPSUtils.DRAW)
			this.winner.setText("It's a draw");
		else if (winner == RPSUtils.PLAYER)
			this.winner.setText("You win");
		else if (winner == RPSUtils.COMPUTER)
