...
public class Turtle {
	...
	public Turtle(Point2 position, double angle, Color c) {
		...
		if (!position.similarTo(new Point2(0.5, 0.5), 0.5))
	...
	public void move(double distance) {
		Vector2 scaled = Vector2.dirVector(angle).multiply(distance);
		Point2 oldPosition = new Point2(this.position);
		position.add(scaled);
		StdDraw.setPenColor(getPenColor());
		StdDraw.line(oldPosition.getX(), oldPosition.getY(), this.position.getX(), this.position.getY());
	}
	...
	public void setPenColor(Color c) {
		penColor = new Color(c.getRed(), c.getGreen(), c.getBlue(), c.getAlpha());
	}
